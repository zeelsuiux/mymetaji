...
function panel_tenant_code() {
    $candidate = isset($_GET['tenant']) ? (string)$_GET['tenant'] : (string)($_SESSION['panel_tenant'] ?? '');
    if ($candidate === '' && isset($_SERVER['REQUEST_URI'])) {
        if (preg_match('#/(cst_[0-9]+)(?:/|$)#i', (string)$_SERVER['REQUEST_URI'], $match)) {
            $candidate = $match[1];
        }
    }
    $candidate = strtolower(trim($candidate));
    if ($candidate !== '' && !preg_match('/^cst_[0-9]+$/', $candidate)) return '';
    if ($candidate !== '') $_SESSION['panel_tenant'] = $candidate;
    return $candidate;
}

function panel_database_dir() {
    $tenant = panel_tenant_code();
    return $tenant !== '' ? __DIR__ . '/shops/' . $tenant . '/database' : __DIR__ . '/database';
}
...
function db_file($module) {
    return panel_database_dir() . '/' . preg_replace('/[^a-zA-Z0-9_-]/', '', $module) . '.json';
}

function db_init() {
    if (!is_dir(panel_database_dir())) mkdir(panel_database_dir(), 0755, true);
    $modules = db_modules();

...
if ($action === 'new' && $leadId) {
    ...
    if ($lead && ($lead['status'] ?? 'New') === 'New') {
        ...
        $leadPasswordHash = (string)($lead['password_hash'] ?? '');
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id === 0) {
        if ($form['password'] === '' && $leadPasswordHash === '') $errors[] = 'Password is required for a new customer.';
    ...
    if (!$errors && $id === 0) {
        // --- Create new customer + first subscription + provision tenant data ---
        $customerId = mdb_insert('customers', [
            'name' => $form['name'], 'phone' => $form['phone'], 'email' => $form['email'],
            'address' => $form['address'], 'username' => $form['username'],
            'password_hash' => $form['password'] !== '' ? password_hash($form['password'], PASSWORD_DEFAULT) : $leadPasswordHash,
            'slug' => '',
        ]);
        ...
        if ($leadId) mdb_update('leads', $leadId, ['status' => 'Converted', 'customer_id' => $customerId]);
    ...
    <div class="panel-body">
        ...
        <form method="post">
            ...
            <div class="form-grid">
                ...
                <div class="form-group"><label>Password <?= $id ? '(leave blank to keep current)' : ($leadId ? '(use signup password or enter a new one)' : '*') ?></label><input type="password" name="password" <?= ($id || $leadId) ? '' : 'required' ?>></div>
            </div>

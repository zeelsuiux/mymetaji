...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($formAction === 'renew') {
        ...
        if (!$errors) {
            $latestBeforeRenewal = customer_latest_subscription($id);
            $expiryDate = calc_renewal_expiry_date($latestBeforeRenewal['expiry_date'] ?? '', $purchaseDate);
            $subscription = [
                'amount' => (float)$amount,
...
$panelPath = customer_panel_path($customer['slug'] ?? '');
    ...
    <div class="panel-header">
        ...
        <div class="toolbar">
            ...
            <?php if (!empty($customer['slug'])): ?>
                <a class="btn btn-outline" href="<?= htmlspecialchars($panelPath) ?>" target="_blank"><?= icon('building', 14) ?> Open Shop Panel</a>
            <?php endif; ?>
        </div>
    </div>

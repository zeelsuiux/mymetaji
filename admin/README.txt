...
/shops/cst_<id>/     -> Dareke customer nu alag database ane assets folder.
/root PHP files       -> Badha customer mate ek j shared panel code.

UPLOAD KEVI RITE KARVU
-----------------------
1. Aa akhi "final" folder ni andar ni badhi files/folders (master, shops,
   root PHP files, .htaccess) tamara hosting na public_html (ke jyare pan
...
3. `database` folder ma PHP ne likhvani permission joiye - jarur padhe to
   `master/database` ane `shops/*/database`
...
PUBLIC SIGNUP + LEAD CONVERSION
-------------------------------
1. Public registration URL: https://yourdomain.com/signup.php
2. Customer signup form submit kare etle data Master Panel > Signup Leads ma
   pending lead tarike save thase. Customer nu password secure hash tarike save
   thay che; customer tarat login kari shakto nathi.
3. Master Panel ma Signup Leads kholo ane Convert par click karo.
4. Customer ni subscription amount, payment type ane purchase date bharo.
   Save karta customer create thase, cst_<id> tenant folder banse, ane lead
   Converted status ma aavse.

TRIAL + RAZORPAY RENEWAL
------------------------
1. Navo public signup submit thata customer ne automatic 15-day Trial male che;
   Master approval vagar panel use kari shake che.
2. Trial/renewal amount `razorpay_config.php` ni `RAZORPAY_PLAN_AMOUNT` value
   mathi levay che (haal ₹5000 set che).
3. Trial expire thay pachi customer existing data joi shake che, pan add/edit/
   delete/export lock thai jase. Expired page par Razorpay Checkout button aavse.
4. Razorpay Dashboard mathi `RAZORPAY_KEY_ID` ane `RAZORPAY_KEY_SECRET`
   `razorpay_config.php` ma set karo. Payment signature ane captured status
   server par verify thay pachi 1 year subscription automatic activate thase.
...
4. "Save Customer" dabao etle:
   - Ek navo tenant /cst_<customer-id>/ automatic ban jase (e.g. /cst_1/)
   - Tena Admin login (username/password) e j hase jem tame nakhyu
   - License (expiry) pan set thai jase

Shopkeeper ni panel URL hase: https://yourdomain.com/cst_<customer-id>/
...
4. Dareke shopkeeper ni data tenna potana /shops/cst_<id>/database/ folder ma
